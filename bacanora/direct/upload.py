...
def put(file_path,
        ...
        atomic=True,
        agave=None):
    """Emulate a Tapis upload by copying a file to its resolved physical
    location to the local host
    """
    try:
        file_name = os.path.basename(file_path)
        posix_path = abs_path(
            destination_dir, system_id=system_id, agave=agave)
        ...
        try:
            ...
            raise DirectOperationFailed('Copy {} failed'.format(temp_fname),
                                        exc)
            ...
            try:
                if settings.DEBUG_MODE is False:
                    os.rename(
                        ...
                        os.path.join(posix_path, os.path.basename(file_path)))
                else:
                ...
                raise DirectOperationFailed('Rename failed after upload', rexc)
